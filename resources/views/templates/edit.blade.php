...
@push('scripts')
    ...
    <script>
        ...
        var deleteFileFromRepairablityDropZoneRoute = "{{ route('templates.page.repairability.delete-file') }}"
        ...
        $(document).ready(function() {
            ...
            // });
            $(document).on('click', '.tab-item', function() {
                // Remove bg-blue-400 from all tabs and add bg-blue-200
                $(".tab-item").removeClass("bg-blue-400").addClass("bg-blue-200");
                    ...
                    success: function(response) {
                        ...
                        <div id="tab${response.page.id}" class="tab-content hidden bg-blue-50 p-4 rounded shadow mb-4">
                            <div class="flex items-center justify-between">
                                ...
                                <button class="text-blue-500 hover:text-blue-600 edit-button" data-id="${response.page.id}" data-name="${response.page.name}">Edit</button>
                            </div>
                            <div id="editPageForm-${response.page.id}" class="edit-form hidden mb-2">
                                <input type="text" id="editInput-${response.page.id}" class="border rounded p-2 w-full"
                                    value="${response.page.name}" />
