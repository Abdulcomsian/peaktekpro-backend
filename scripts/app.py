...
class PDFSignatureExtractor:
    ...
    def is_signature(self, image):
        """
        Signature detection using the EXACT same logic as the working Flask app
        """
        try:
            ...
            # EXACT same heuristics as Flask app
    ...
    def extract_signatures(self, pdf_path, include_base64=True, save_images=True):
        """
        Extract signatures from PDF using the EXACT same logic as the working Flask app
        """
        signatures = []
        ...
        try:
            ...
            for page_num in range(len(doc)):
                ...
                # First try extracting embedded images (EXACT same as Flask app)
                if image_list:
                        ...
                        try:
                            ...
                            if self.is_signature(img_cv):
                                signature_data = {
                                    ...
                                    'confidence': 0.8  # Default confidence for embedded images
                                }
                ...
                # If no embedded images found OR no signatures found in embedded images, 
                # try rendering the page (EXACT same logic as Flask app)
                page_has_embedded_sigs = any(
                    sig['page'] == page_num + 1 and sig['type'] == 'embedded' 
                    for sig in signatures
                )
                
                if not image_list or not page_has_embedded_sigs:
                    try:
                        # Use same DPI as Flask app
                        ...
                        if self.is_signature(img_cv):
                            signature_data = {
                                ...
                                'width': img_cv.shape[1],
                                'height': img_cv.shape[0],
                                'confidence': 0.6  # Lower confidence for rendered pages
                            }
