    ...
    public function updateInprogress200(Request $request, $jobId)
    {
        ...
        try {
            ...
            foreach ($imageCategories as $category) {
                ...
                $uploadedLabels = collect($request->$category)->pluck('label')->filter()->map(function ($label) {
                    return strtolower(trim($label)); // Normalize for case-insensitive comparison
                });
            
                foreach ($existingMedia as $media) {
                    if (!$uploadedLabels->contains(strtolower(trim($media->labels)))) {
                        if($media->image_path) //chaeck the image is not null
                        {
                            Storage::delete($media->image_path); // Delete the associated file
                        }
                        $media->delete(); // Remove database entry
                    }
                }
            
                if (isset($request->$category) && is_array($request->$category)) {
                    foreach ($request->$category as $imageData) {
                        $label = isset($imageData['label']) ? strtolower(trim($imageData['label'])) : null;
                        ...
                        InprogressMedia::updateOrCreate(
                            [
                                'company_job_id' => $jobId,
                                'category' => $category,
                                'labels' => $label, // Save `null` if no label is provided
                            ],
                            [
                                'image_path' => $url, // Save `null` if no image is provided
                            ]
                        );
                    }
                }
